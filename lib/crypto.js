...
function RSADoPublic(x) {
  return x.modPowInt(this.e, this.n);  //me mod n 
}
...
function encrypt(pkey, data) {
  var base64Str = Base64.encode(data);
  var splitcnt = Math.ceil(base64Str.length/50);
  var encrypted = "";
  
  var rsa = new RSAKey();
  rsa.n = parseBigInt(pkey, "16");
  rsa.e = parseInt("010001", 16);

  for (var i = 0; i < splitcnt; i++) {
    var pos = i * 50;
    var end_pos = (i == splitcnt-1) ? base64Str.length : pos + 50;
    encrypted += rsa.encrypt(base64Str.substring(pos, end_pos));
  }
